...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateCurrency() {
        ...
        if (currencyLabel) {
            currencyLabel.textContent = isUsd ? 'USD' : 'KZT';
        }

        document.querySelectorAll('.currency-value').forEach(el => {
            const KZTValue = parseFloat(el.dataset.KZTValue);
            if (isNaN(KZTValue)) return;

            let displayValue;
            let originalText = el.textContent || "";
            let suffix = originalText.includes("KZT/м²") ? " USD/м²" : (originalText.includes("KZT") ? " USD" : "");

            if (isUsd) {
                displayValue = '$ ' + (KZTValue / usdRate).toLocaleString('ru-RU', { maximumFractionDigits: 0 });
                if(suffix) displayValue = displayValue.replace(" USD", suffix);
            } else {
                displayValue = KZTValue.toLocaleString('ru-RU', { maximumFractionDigits: 0 });
                if(suffix) displayValue += suffix.replace("USD", "KZT");
                else displayValue += " KZT";
            }
