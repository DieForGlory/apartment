    ...
    function updateCurrency(isUsd) {
        ...
        document.querySelectorAll('.currency-value').forEach(el => {
            ...
            if (isUsd) {
                ...
                let usdValue = KZTValue / usdRate;
                displayValue = '$' + usdValue.toLocaleString('en-US', { maximumFractionDigits: 0 });
            } else {
                if (currencyLabel) currencyLabel.textContent = 'KZT';
                displayValue = KZTValue.toLocaleString('ru-RU', { maximumFractionDigits: 0 }).replace(/,/g, '.');
            }
        ...
        if (exportLink) {
            ...
            url.searchParams.set('currency', isUsd ? 'USD' : 'KZT');
