...
# --- МОДЕЛИ ДЛЯ КОНСТРУКТОРА КАЛЬКУЛЯТОРОВ ---
...
class PaymentTemplate(db.Model):
    ...
    min_initial_payment_percent = db.Column(db.Float, nullable=False)
    min_nrv_percent = db.Column(db.Float, nullable=False)
    discount_rate_annual = db.Column(db.Float, nullable=False, default=16.5)

    # Новый столбец для хранения прогрессивных ставок: {"1": 17.0, "2": 20.0, "3": 21.0, "4": 22.0}
    discount_rates_json = db.Column(db.Text, nullable=True)

    is_active = db.Column(db.Boolean, default=True)
    require_nrv_validation = db.Column(db.Boolean, default=True, nullable=False)


class CustomPaymentSchedule(db.Model):
    __bind_key__ = 'planning_db'
    __tablename__ = 'custom_payment_schedules'

    id = db.Column(db.Integer, primary_key=True)
    sell_id = db.Column(db.Integer, nullable=False, index=True)
    template_id = db.Column(db.Integer, db.ForeignKey('payment_templates.id'), nullable=False)
    month_index = db.Column(db.Integer, nullable=False)
    payment_amount = db.Column(db.Integer, nullable=False)
    payment_date = db.Column(db.Date, nullable=False)

    template = db.relationship('PaymentTemplate')
