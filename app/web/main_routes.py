...
from ..models.planning_models import PropertyType, PaymentMethod, PaymentTemplate # Добавлен PaymentTemplate
...
def selection():
    ...
    return render_template('main/selection.html',
                           title=_("Qualified по бюджету"),  # <-- Тоже переводим заголовок
                           results=results,
...
def apartment_details(sell_id):
    ...
    # Достаем шаблоны для конструктора
    ...
    return render_template(
        ...
        templates=templates,  # Передаем шаблоны в Frontend
...
def generate_commercial_offer(sell_id):
    ...
    # Убираем логику extended/standard. Выводим то, что есть в базовых расчетах.
    card_data['pricing'] = updated_pricing_for_template

    current_date = datetime.now().strftime("%d.%m.%Y %H:%M")
    usd_rate_from_cbu = currency_service.get_current_effective_rate()
    fallback_usd_rate = current_app.config.get('USD_TO_KZT_RATE', 12650.0)
    ...
    return render_template(
        ...
        usd_to_KZT_rate=actual_usd_rate,
