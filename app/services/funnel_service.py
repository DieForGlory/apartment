...
def get_target_funnel_metrics(start_date_str: str, end_date_str: str):
    ...
    # --- Конверсии из "Qualifiedа" ---
    podbor_base_ids = {pid for pid in target_ids if lead_has_status(pid, 'Qualified')}
    podbor_to_otkaz_ids = {pid for pid in podbor_base_ids if lead_has_status(pid, 'Отказ')}
    podbor_to_vstrecha_ids = {pid for pid in podbor_base_ids if lead_has_status(pid, 'Qualified', 'Назначенная встреча')}
    ...
    vstrecha_base_ids = {pid for pid in target_ids if lead_has_status(pid, 'Qualified', 'Назначенная встреча')}
    vstrecha_to_sostoyalas_ids = {pid for pid in vstrecha_base_ids if lead_has_status(pid, 'Qualified', 'Визит состоялся')}
    vstrecha_to_nesostoyalas_ids = {pid for pid in vstrecha_base_ids if lead_has_status(pid, 'Qualified', 'Визит не состоялся')}
    ...
    vizit_sostoyalsya_base_ids = {pid for pid in vstrecha_base_ids if lead_has_status(pid, 'Qualified', 'Визит состоялся')}
