...
def _get_our_project_dynamic_stats(complex_name, property_type_russian):
    ...
    # 2. Остатки (Маркетинговый резерв и Qualified)
    remainders = EstateSell.query.filter(
        EstateSell.house_id.in_(house_ids), EstateSell.estate_sell_category == mysql_key,
        EstateSell.estate_sell_status_name.in_(["Маркетинговый резерв", "Qualified"]),
    ...
    if remainders:
        ...
        sum_area, sum_price_sqm_KZT, sum_bottom_sqm_KZT = 0, 0, 0
        for r in remainders:
            sum_area += r.estate_area
            sum_price_sqm_KZT += (r.estate_price / r.estate_area)
            # Цена дна: (Цена - Вычет) * (1 - Скидки)
            bottom_price = (r.estate_price - deduction) * (1 - total_discount_rate)
            sum_bottom_sqm_KZT += (bottom_price / r.estate_area)

        avg_area = sum_area / len(remainders)
        avg_price_sqm = (sum_price_sqm_KZT / len(remainders)) / usd_rate
        avg_bottom_price = (sum_bottom_sqm_KZT / len(remainders)) / usd_rate
