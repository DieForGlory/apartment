...
REMAINDER_STATUSES = ["Маркетинговый резерв", "Qualified"]
...
def calculate_required_avg_price(project_name: str, property_type_str: str, start_date_str: str):
    ...
    # 2. Рассчитать общий объем контрактации (KZT) с даты начала ДЛЯ ЭТОГО ТИПА
    effective_date = func.coalesce(EstateDeal.agreement_date, EstateDeal.preliminary_date)
    total_volume_KZT_query = mysql_session.query(
    ...
    total_volume_KZT = total_volume_KZT_query.scalar() or 0.0
    ...
    total_volume_usd = total_volume_KZT / usd_rate
