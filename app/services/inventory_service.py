...
def generate_commercial_inventory_excel(currency: str, usd_rate: float):
    ...
    commercial_sells = mysql_session.query(EstateSell).options(
        ...
        EstateSell.estate_sell_status_name.in_(["Маркетинговый резерв", "Qualified", "Бронь"]),
...
def get_inventory_summary_data():
    ...
    valid_statuses = ["Маркетинговый резерв", "Qualified", "Бронь"]
...
def generate_inventory_excel(summary_data: dict, currency: str, usd_rate: float):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        money_format_KZT = workbook.add_format({'num_format': '#,##0', 'border': 1})
        ...
        current_money_format = money_format_usd if is_usd else money_format_KZT
