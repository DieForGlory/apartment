...
def generate_kpi_report_excel(year: int, month: int):
    ...
    for manager in managers:
        ...
        kpi_bonus_KZT = calculate_manager_kpi(plan.plan_income, fact_income)

        source_data.append({
            "full_name": manager.full_name,
            "plan_KZT": plan.plan_income,
            "fact_KZT": fact_income,
            "kpi_bonus_KZT": kpi_bonus_KZT,
            "kpi_bonus_usd": kpi_bonus_KZT / usd_rate
        })
    ...
    for i, data in enumerate(source_data):
        final_report_rows.append({
            ...
            'Личный план продаж на период (долл. США)': data['plan_KZT'] / usd_rate,
            'Факт выполнения личного плана продаж на период (долл. США)': data['fact_KZT'] / usd_rate,
            '% выполнения личного плана продаж': (data['fact_KZT'] / data['plan_KZT']) if data['plan_KZT'] > 0 else 0,
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        money_KZT_format = workbook.add_format({'num_format': '#,##0', 'border': 1})
        ...
        worksheet.set_column('I:I', 25, money_KZT_format)
        worksheet.set_column('J:J', 25, money_KZT_format)

        # 7. Вставляем формулы
        for idx, data in enumerate(source_data):
            row_num = idx + 3
            kpi_usd = data['kpi_bonus_usd']
            kpi_KZT = data['kpi_bonus_KZT']

            worksheet.write_formula(f'H{row_num}', f'=IF(ISBLANK(G{row_num}),0,{kpi_usd}*G{row_num})')
            worksheet.write_formula(f'I{row_num}', f'=IF(ISBLANK(G{row_num}),0,{kpi_KZT}*G{row_num})')
