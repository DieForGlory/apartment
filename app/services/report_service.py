...
def get_fact_income_data(year: int, month: int, property_type: str):
    ...
    for complex_name, summa, op_date in query.all():
        ...
        if settings.use_historical_rate:
            # Деление суммы в KZT на курс даты операции для получения валютного эквивалента
...
def process_plan_from_excel(file_path: str, year: int, month: int):
    ...
    for index, row in df.iterrows():
        ...
        plan_entry.plan_volume = row['План контрактации, KZT']
        plan_entry.plan_income = row['План поступлений, KZT']
...
def generate_plan_template_excel():
    ...
    headers = ['ЖК', 'Тип недвижимости', 'План, шт', 'План контрактации, KZT', 'План поступлений, KZT']
    data = [{'ЖК': name, 'Тип недвижимости': prop_type.value, 'План, шт': 0, 'План контрактации, KZT': 0,
             'План поступлений, KZT': 0} for name in complex_names for prop_type in property_types]
...
def generate_plan_fact_excel(year: int, month: int, property_type: str, period: str = 'monthly', currency: str = 'KZT',
                             rate: float = 1.0):
    """
...
def generate_annual_report_excel(year: int, currency: str = 'KZT', rate: float = 1.0):
